...
## Description
Create `test/integration/navigation.test.ts` covering the four navigation tools:
`browser_navigate`, `browser_go_back`, `browser_go_forward`, and `browser_close`.
...
| I2 | `browser_go_back` | Navigate to test-page, then second-page; call go_back; verify response or subsequent snapshot references test-page content |
| I3 | `browser_go_forward` | After I2 sequence (back to test-page), call go_forward; verify response references second-page |
...
- [x] Given the browser has visited `test-page.html` then `second-page.html`, when
  `browser_go_back` is called, then the response text is "Navigated back" (no isError).
- [x] Given the browser has gone back to `test-page.html`, when `browser_go_forward`
  is called, then the response text is "Navigated forward" (no isError).
...
- **Criterion 2** (browser_go_back → "Navigated back"): PASS — Test I2 navigates test-page → second-page then calls `browser_go_back` and asserts `text.toContain('Navigated back')` with `isError` falsy. Passed.
- **Criterion 3** (browser_go_forward → "Navigated forward"): PASS — Test I3 calls `browser_go_forward` after I2's back and asserts `text.toContain('Navigated forward')` with `isError` falsy. Passed.
