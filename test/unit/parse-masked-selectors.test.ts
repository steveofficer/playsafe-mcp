...
describe("parseMaskedSelectors", () => {
  ...
  test("U2: multiple --mask flags return all selectors", () => {
    expect(
      parseMaskedSelectors(["--mask", ".btn", "--mask", "#panel"]),
    ).toStrictEqual([".btn", "#panel"]);
  });
  ...
  test("U5: whitespace around selector is trimmed", () => {
    expect(parseMaskedSelectors(["--mask", "  .btn  "])).toStrictEqual([
      ".btn",
    ]);
  });
  ...
  test("U7: unrelated flags before --mask are ignored", () => {
    expect(
      parseMaskedSelectors(["--other", "val", "--mask", ".x"]),
    ).toStrictEqual([".x"]);
  });
